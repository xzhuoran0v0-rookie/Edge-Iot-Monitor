...
children.push(p("故引入判据：连续带外时长达到 1 小时即判定为环境迁移，清零重新学习；任一带内样本使计时归零。该判据按时钟度量而非按样本计数（见 3.4 节），因此处理周期自适应变化不会改变“多久算持续偏离”这一语义。由 2.2 节结论，该机制不影响告警行为。"));
...
children.push(h2("3.4  周期的分离，与其中唯一可变的一档"));
children.push(p("系统内共有四个速率，各自由其物理或经济约束决定，不合并为一个参数："));
children.push(table(
  ["周期", "取值", "驱动", "约束来源"],
  [
    ["安全采样", "1 s，固定", "硬限与告警阈值判定，驱动蜂鸣器", "告警时延要求"],
    ["处理与上报", "2-10 s，自适应", "滤波、基线、OLED 刷新、本地服务上报", "下界为 SHT30 响应 τ63≈2 s；上界见下"],
    ["EDGE_SAMPLE_INTERVAL_MS", "10 s，固定", "向 EdgeReasoner 输入样本", "12 样本 × 10 s = 2 min 趋势窗口"],
    ["CLOUD_INTERVAL_MS", "60 s，固定", "IoTDA 属性上报", "云平台消息配额"],
  ], [26, 10, 30, 34]));
children.push(gap());
children.push(p("合并为单一参数将迫使这些约束相互妥协。以处理与上报为例：缩短单一参数可提升显示响应，但云端消息量同比上升。分离后每日上报 1 440 条，占免费额度（10 000 条/日）的 14%，同一额度可支撑约 6 台设备；若取 10 s，单台即占用 86%，第二台设备便会超限。"));
children.push(p("推理输入之所以另设一档，是因为 EdgeReasoner 的窗口按**样本数**固定为 12，而其阈值是按 **2 分钟**窗口整定的。若直接按处理周期输入，该周期一旦自适应，窗口时长便随环境忽长忽短：阈值未变而含义已变——净升条件将取代速率条件成为实际门槛，判定“快速上升”所需的斜率反而被抬高。这也正是自适应上界取 EDGE_SAMPLE_INTERVAL_MS 的原因：上报再慢，推理窗口就喂不满了。"));
children.push(p("**四档之中只有一档允许自适应。** 其判据取自基线已学到的平均偏差——本场所平时的波动幅度。偏差小则上报间隔向 10 s 拉长；出现状态变迁、严重度非 info、基线未就绪、带外或越硬限中的任何一项，立即回到 2 s 并锁定 30 s。变慢每周期仅 +500 ms 且随时可被打断，变快一步到位——该不对称使判断失误的代价落在“多处理几次”一侧，而非“发现得晚”一侧。"));
children.push(p("需要强调的是，**自适应的是处理与上报，不是采样**。传感器仍由 1 Hz 安全任务读取，一次不少，因为采样频率即告警时延。这一档之所以敢于可变，恰恰因为告警不经过它：把硬限判定放进独立 1 Hz 任务是本系统早期的架构决定，其回报正体现于此。"));
children.push(p("自适应还要求基线的两个判据改为按时钟度量。预热与重学在实现上原本按样本计数，其语义却是时长；周期固定时二者可由周期反推换算，周期一旦可变，换算即失效。故 observe() 改为接收 millis()，预热按累计 4 min、重学按连续带外 1 h 直接判定，且单次调用最多计入 60 s。"));
...
children.push(p("室温 24–27 °C，相对湿度 43–48 %RH；告警阈值 30 °C / 70 %RH；处理与上报周期 2–10 s 自适应，云端上报周期 60 s。"));
...
children.push(p("**存储行为。** 设备每个上报周期发送一次判决，而 edge_assessments 表仅记录状态变迁：实测 8 次上报产生 3 行记录，每行时间戳即该状态的起始时刻。"));
...
children.push(p("单设备每日至多产生 43 200 次本地判决（2 s 下界），环境安静时随上报周期拉长而减少，全部在设备上完成，无计费开销；上行云端消息 1 440 条，占免费额度 14%。"));

// ---------------- 5 ----------------
children.push(h1("5  结果分析与结论"));
children.push(p("实测表明，将判决下沉至 MCU 的方案达到了设计目标。**告警可用性与网络解耦**：断网、云端不可达、冷启动进入异常环境三种条件下，告警时延与联网时一致（≤ 1 s）；这一性质来自架构而非补救，判决路径中不存在网络调用。**适应性与安全性可以共存**：自适应带被钳制在固定硬限内侧，且告警阈值不参与学习，因此允许基线随环境移动不会削弱任何安全保证——会自适应的部分与提供保证的部分被显式分开，且前者被后者约束。**开销可忽略**：RAM 与 Flash 占用均低于 15%，其中推理本身不足 0.2 KB。**学到的东西有两个去处**：基线学到的偏差不仅用于判定是否偏离常态，还用于决定处理与上报的频度，而采样、硬限与告警始终固定于 1 Hz——采样频率即告警时延，这一档不可让渡。"));
...
children.push(p("其中一项已具备明确的实施路径：**令采样本身也可自适应**。当前采样固定 1 Hz，因为该频率即告警时延。两全的做法是启用 SHT30 自带的 ALERT 引脚——将告警阈值写入芯片的限值寄存器，越限由传感器硬件中断通知 MCU，MCU 便可降低采样率，且时延优于 1 Hz 轮询。所用模块已将该引脚引出（丝印 AL），所缺者为一根至空闲 GPIO 的连线与限值寄存器配置。本版未纳入，是因为 ALERT 仅在周期测量模式下有效，启用它需改动当前已验证的采样路径。"));
...
children.push(table(
  ["参数", "取值", "位置"],
  [
    ["安全采样 / 处理上报 / 推理输入 / 云端上报", "1 s / 2-10 s / 10 s / 60 s", "senseIntervalMs 等"],
    ["快照最大陈旧时间", "1.5 s", "SAFETY_SAMPLE_MAX_AGE_MS"],
    ["推理窗口", "12 样本（跨 2 min）", "WINDOW_SIZE"],
    ["基线预热 / 重学判据", "4 min / 连续带外 1 h", "按时钟计：warmupMs 等"],
